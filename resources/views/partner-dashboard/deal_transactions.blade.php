...
<div class="container-fluid">
    <div class="row bg-title">
        ...
        <a href="{{url('partner/branch/transactions')}}" class="btn btn-primary">Back</a>
    </div>
    ...
    <div class="row">
        <div class="col-md-12 col-xs-12">
            <form class="form-inline" action="{{url('partner/branch/sort_deal_purchased')}}" method="post" class="mb">
                {{csrf_field()}}
                ...
                <div class="table-responsive">
                    <table id="transactionList" class="table table-bordered table-hover table-striped projects">
                        ...
                        @foreach ($transactions as $row)
                            <tr>
                                <td><img src="{{$row->customer_profile_image}}" width="100%" class="pro_pic" alt="post-image"></td>
                                <td>{{ $row->customer_full_name }}</td>
                                <td>{{ $row->heading }}</td>
                                <!-- <td>Merchant</td> -->
                                <td>{{ date("d F, Y", strtotime($row->posted_on)) }}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
