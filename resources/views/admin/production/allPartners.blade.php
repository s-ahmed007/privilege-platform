...
<div class="right_col" role="main">
   <div class="page-title">
      <div class="title_left">
         ...
         <div class="alert alert-danger">{{ session('delete branch') }}</div>
         @elseif (session('delete partner'))
         <div class="alert alert-danger">{{ session('delete partner') }}</div>
         @elseif(session('try_again'))
         <div class="alert alert-warning">{{ session('try_again') }}</div>
         @elseif(session('main_branch_deactivate_msg'))
         <div class="alert alert-warning">{{ session('main_branch_deactivate_msg') }}</div>
         @endif

        <h3>All Partner Branches</h3>
        <a class="btn btn-all" href="{{ url('allPartners/activated') }}">Active</a>
        <a class="btn btn-inactive" href="{{ url('allPartners/deactivated') }}">Deactive</a>
                     ...
                     @if(isset($allPartners))
                        <?php $i=1; ?>
                         ...
                         <tr>
                            ...
                            <td>{{$partner->partner_email != '0' ? $partner->partner_email : 'No e-mail'}}<br>
                            ...
                            ?>
                            <td style="background-color: {{$color}}" title="{{$text}}">
                                {{ date("F d, Y", strtotime($value->info->expiry_date)) }}
                            </td>
                            ...
                            <td align="center">
                                <select id="partner_edit_{{$partner->id}}" class="selectChangeOff"
                                    ...
                                    <option value="2">Profile Img</option>
                                    <option value="3">Gallery Img</option>
                                    ...
                                    <option value="8">Cover Img</option>
                                    <option value="9">Menu</option>
