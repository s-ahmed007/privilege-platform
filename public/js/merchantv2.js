...
{
    var url = base_url + "/" + "partner/checkUser";
        ...
        success: function (data) {
            if (data.error) {
                ...
                data.offers.forEach(function (offer) {
                    ...
                    if (typeof offer.redeem !== 'undefined') {
                        output += '<span style="padding: 2px; background-color: #ffc107; border-radius: 3px">Reward</span>\n';
                    }
                    output += "</div>\n";
                    ...
                    if (!offer.expired) {
                        output +=
                            '<button class="pull-right disable_btns btn btn-' + btn_color + '" id="offer_' +
                            offer.id +
                        ...
                        output += '<button class="pull-right btn btn-danger">EXPIRED</button>\n';
                    }
                    output += "</div>\n";
                    output += "</div>\n";
                    output += "</li>";
...
// confirm checkout
function confirmCheckout(customerId, offerId, redeem_id, elem)
{
    var conf_text = redeem_id === null ? 'Are you sure to avail this offer?' : 'Are you sure to redeem this reward?';
    if (confirm(conf_text)) {
        //make button inactive
        elem.disabled = true;
        elem.innerText = "Submitting...";
        $('.disable_btns').prop('disabled', true);

        var url = base_url + "/" + "partner/confirm_offer_transaction/";
        axios
            .post(url, {
                user_id: customerId,
                offer_id: offerId,
            ...
            .then(function (response) {
                if (response.status === 200) {
                    window.location = base_url + "/" + "partner/branch/transactions";
...
customer_id.addEventListener("keyup", function(event) {
    if (event.keyCode === 13) {
        ...
        document.getElementById("checkout_user").click();
    }
});
