...
<div class="container offers-container">
    ...
    <div class="row">
        ...
        <div class="col-md-9 col-sm-12 col-xs-12 offers">
            {{--sub cat 2 block--}}
            <br>
            @if($sub_cat_2 == 'show' || $sub_cat_2 != 'hide')
                @if(count($all_sub_cats_2) > 6)
                    <button class="btn btn-activate pull-right" onclick="hideCategory2(this)"><i class="fa fa-angle-down"></i></button>
                @endif
                <?php
                $cat2_div_1 = $all_sub_cats_2->take(6);
                $cat2_div_2 = $all_sub_cats_2->slice(6);
                ?>
                <div class="row">
                    @foreach($cat2_div_1 as $subcategory)
                        <div class="col-md-2 col-sm-2 col-xs-4 category-block">
                            <a href="{{url('offers_copy/'.$selected_category->type.'/'.$sub_cat_1.'/'.$subcategory->cat_name)}}" style="color:#15e2cf;">
                                <div class="" style="display: inline-block;">
                                    <img src="{{$subcategory->icon}}" alt="Royalty BD category" class="lazyload" width="50" height="50">
                                    <p class="" style="display: inline-block;">{{$subcategory->cat_name}}</p>
                                </div>
                            </a>&nbsp;&nbsp;
                        </div>
                    @endforeach
                    <div class="hide_cat_div2">
                        @foreach($cat2_div_2 as $subcategory)
                            <div class="col-md-2 col-sm-2 col-xs-4 category-block">
                                <a href="{{url('offers_copy/'.$selected_category->type.'/'.$sub_cat_1.'/'.$subcategory->cat_name)}}" style="color:#15e2cf;">
                                    <div class="" style="display: inline-block;">
                                        <img src="{{$subcategory->icon}}" alt="Royalty BD category" class="lazyload" width="50" height="50">
